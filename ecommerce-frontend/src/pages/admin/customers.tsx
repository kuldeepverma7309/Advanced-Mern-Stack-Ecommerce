...
const deleteHandler = async(userId:string)=>{
  ...
  //we do not want to navigate therefor we are using null;
...
useEffect(() => {
  if(data) setRows(data.users.map((i)=>({
    avatar: <img style={{borderRadius:"50%",}} src={i.photo} alt="Avatar" />,
    name: i.name,
    email: i.email,
    gender: i.gender,
    role: i.role,
  ...
  return (
    <div className="admin-container">
      <AdminSidebar />
      <main>{isLoading ? <Skelton count={20} /> : Table}</main>
    </div>
  );
};
