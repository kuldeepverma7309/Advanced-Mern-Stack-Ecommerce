...
export const getUser = TryCatch(async (req, res, next)=>{
  const { id } = req.params as { id: string }; //(req.params as { id: string }) explicitly asserts that req.params should have a property named id of type string. This can help TypeScript understand the structure of req.params better.
  const user = await User.findById(id);
  if (!user) return next(new ErrorHandler("Invaild Id", 400));
  return res.status(200).json({
    success: true,
    user,
  });
