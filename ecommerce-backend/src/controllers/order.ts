...
export const newOrder = TryCatch(async(req:Request,res:Response,next:NextFunction)=>{
    const {
      shippingInfo,
      orderItems,
      user,
...
export const getAllOrders = TryCatch(async(req,res,next)=>{
    ...
    if(nodeCache.has("all-orders")){
        ...
        if(!orders) return next(new ErrorHandler("orders not fouond", 404));
...
export const getSingleOrder = TryCatch(async(req,res,next)=>{
    ...
    if(nodeCache.has(`order-${id}`)){
        ...
        if(!order) return next(new ErrorHandler("order not fouond", 404));
